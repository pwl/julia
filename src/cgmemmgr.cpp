...
static void *alloc_shared_page(size_t size, size_t *id, bool exec)
{
    ...
    long unsigned int file_mode = exec ? PAGE_EXECUTE_READWRITE : PAGE_READWRITE;
    ...
    *id = (size_t)hdl;
    ...
    long unsigned int map_mode = FILE_MAP_ALL_ACCESS | (exec ? FILE_MAP_EXECUTE : 0);
    void *addr = MapViewOfFile(hdl, map_mode, 0, 0, size);
    assert(addr && "Cannot map RO view");
    long unsigned int protect_mode = exec ? PAGE_EXECUTE_READ : PAGE_READONLY;
